...
管理员可以创建用户账号。每个用户拥有独立的“我的设备”页面，只查看和管理自己的设备；可以添加、重命名和移除设备。点击“添加设备”后复制安装命令，在自己的 Linux GPU 服务器上运行，即可自动安装依赖、注册到当前中心并通过隔离的 PM2 启动上报端，无需 sudo。安装链接 15 分钟有效，只能注册一次。
