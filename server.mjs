...
export function createMonitor(config, options = {}) {
  ...
  const hosts = new Map(config.hosts.map(h => [h.id, h]));
  ...
  const assets = new Map(['/app.js', '/style.css', '/slurm.js', '/views.js', '/system.js', '/live-time.js',
    '/pwa.js', '/pwa.css', '/i18n.js', '/sw.js', '/offline.html', '/manifest.webmanifest', '/manifest.en.webmanifest',
  ...
  function session(req) {
    ...
    return value && value.expires > now() ? key : null;
  }
  ...
  const server = http.createServer(async (req, res) => {
    ...
    try {
      ...
      if (path === '/api/slurm/ingest' && req.method === 'POST') {
        ...
        if (!source || !auth.startsWith('Bearer ') || !equal(digest(auth.slice(7)), source.tokenHash)) return reply(res, 401, { error: 'Unauthorized' });
      ...
      if (path === '/api/slurm' && req.method === 'GET') {
        if (!session(req)) return reply(res, 401, { error: 'Unauthorized' });
        const time = now();
        return reply(res, 200, { serverTime: time, sources: [...sources.values()].map(source => {
      ...
      if (path === '/api/ingest' && req.method === 'POST') {
        ...
        if (!host || !auth.startsWith('Bearer ') || !equal(digest(auth.slice(7)), host.tokenHash)) return reply(res, 401, { error: 'Unauthorized' });
      ...
      if (path === '/api/login' && req.method === 'POST') {
        ...
        const candidate = (await derive(body.password, config.passwordSalt, 64)).toString('hex');
        if (!equal(candidate, config.passwordHash) || !equal(body.username, config.username)) return reply(res, 401, { error: '用户名或密码错误' });
        loginLimits.delete('ip:' + ip);
        if (sessions.size >= 1000) sessions.delete(sessions.keys().next().value);
        const token = randomBytes(32).toString('base64url');
        sessions.set(digest(token), { expires: now() + 12 * 3600000 });
      ...
      if (path === '/api/snapshot' && req.method === 'GET') {
        if (!session(req)) return reply(res, 401, { error: 'Unauthorized' });
        const time = now();
        return reply(res, 200, { serverTime: time, refreshSeconds: 5, username: config.username, hosts: [...hosts.values()].map(host => {
...
if (process.env.GPU_MONITOR_CONFIG || process.argv[1] === fileURLToPath(import.meta.url)) {
  const config = JSON.parse(readFileSync(process.env.GPU_MONITOR_CONFIG || join(root, 'config.json'), 'utf8'));
